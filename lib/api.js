...
class EbecoApi extends Homey.SimpleClass {
  ...
  updateDeviceState(updatedState) {
    if (this.username === 'test' && this.password === 'test') {
      ...
      this.mockDevices[this.mockDevices.findIndex(device => device.id === updatedState.id)].temperatureSet = updatedState.temperatureSet;
      return new Promise((resolve, reject) => resolve({ test: true }));
    }
